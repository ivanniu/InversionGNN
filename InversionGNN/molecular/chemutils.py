                ...
                for new_bond in bondtype_list:
                    if ith_substructure_is_atom(substructure_idx):
                        ...
                        new_smiles_batch = add_fragment_at_position(editmol = origin_mol, position_idx = leaf_atom_idx, 
                                                                    fragment = new_substructure , new_bond = new_bond)
                        new_smiles_set = new_smiles_set.union(new_smiles_batch)
                        #if 'c1ccc(Nc2nccc(-c3ncccn3)n2)cc1' in new_smiles_batch:
                            #print(adjacency_weight[leaf_idx][extend_idx])
                            #print(adjacency_weight[extend_idx][leaf_idx])
                            #print(list(np.sort(-node_indicator[extend_idx])))
                            #print(expand_prob)
                            #for substructure_idx in added_substructure_lst:
                                #print(vocabulary[substructure_idx])
                            #print(added_substructure_lst)
                        #print(new_substructure)
                        #print(new_bond)
                        #print(new_smiles_set)
                        #print(set_before_add)
                        #print(new_smiles_set-set_before_add)
                        #exit()

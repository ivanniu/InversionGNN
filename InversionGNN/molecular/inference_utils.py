...
def oracle_screening(smiles_set, oracle):
	...
	smiles_score_lst.sort(key=lambda x:np.mean(x[1]), reverse=True)
	return smiles_score_lst 

def oracle_screening_mrl(smiles_set, oracle, preference):
	smiles_score_lst = []
	for smiles in smiles_set:
		score = oracle(smiles)
		smiles_score_lst.append((smiles, score))

	smiles_score_lst.sort(key=lambda x:np.max(-preference * np.log(x[1])))
	return smiles_score_lst 

def oracle_screening_(smiles_set, oracle):
	smiles_score_lst = []
	for smiles in smiles_set:
		score = oracle(smiles)
		smiles_score_lst.append((smiles, score))
...
def dpp(smiles_score_lst, num_return, lamb):
	...
	score_arr = np.array([np.mean(i[1]) for i in smiles_score_lst])
...
def optimize_single_molecule_one_iterate(smiles, gnn):
	...
	t1 = t.time()
	#node_mask, node_indicator, adjacency_mask, adjacency_weight = smiles2differentiable_graph(smiles)
	(is_nonleaf, is_leaf, is_extend), node_indicator, adjacency_mask, adjacency_weight, leaf_extend_idx_pair, leaf_nonleaf_lst = smiles2differentiable_graph_v2(smiles) #leaf leanable?
	node_mask = is_nonleaf + is_leaf
	#differentiable_graph = gnn.update_molecule(node_mask, node_indicator, adjacency_mask, adjacency_weight)
	t4 = t.time()
	differentiable_graph = gnn.update_molecule_split(node_mask, node_indicator, adjacency_mask, adjacency_weight)
	print(differentiable_graph)
	t2 = t.time()
	#exit()
	smiles_set = differentiable_graph2smiles_v0(origin_smiles = smiles, differentiable_graph = differentiable_graph, 
											 leaf_extend_idx_pair = leaf_extend_idx_pair, leaf_nonleaf_lst = leaf_nonleaf_lst)#no shrink
	t3 = t.time()
	print(t3-t1)
	print(t2-t4)
	return smiles_set
...
#### shrink is not learnable;   greedy
def optimize_single_molecule_one_iterate_epo(smiles, gnn, preference, n_property):
	if not is_valid(smiles):
		return set()
	t1 = t.time()
	#node_mask, node_indicator, adjacency_mask, adjacency_weight = smiles2differentiable_graph(smiles)
	...
	t4 = t.time()
	#differentiable_graph = gnn.update_molecule(node_mask, node_indicator, adjacency_mask, adjacency_weight)
	differentiable_graph = gnn.update_molecule_split_epo(node_mask, node_indicator, adjacency_mask, adjacency_weight, preference, n_property)
	print(differentiable_graph)
	t2 = t.time()
	#exit()
	smiles_set = differentiable_graph2smiles_v0(origin_smiles = smiles, differentiable_graph = differentiable_graph, 
...
def optimize_single_molecule_one_iterate_epo_v3(smiles, gnn, topk, epsilon):
	if not is_valid(smiles):
		return set()
	(is_nonleaf, is_leaf, is_extend), node_indicator, adjacency_mask, adjacency_weight, leaf_extend_idx_pair, leaf_nonleaf_lst = smiles2differentiable_graph_v2(smiles)
	differentiable_graph = gnn.update_molecule_epo_v2((is_nonleaf, is_leaf, is_extend), node_indicator, adjacency_mask, adjacency_weight, leaf_extend_idx_pair, leaf_nonleaf_lst)
...
def optimize_single_molecule_one_iterate_v2(smiles, gnn):
	if not is_valid(smiles):
		return set()
	(is_nonleaf, is_leaf, is_extend), node_indicator, adjacency_mask, adjacency_weight, leaf_extend_idx_pair, leaf_nonleaf_lst = smiles2differentiable_graph_v3(smiles) #leaf fixed?
	...
	smiles_set = differentiable_graph2smiles(origin_smiles = smiles, differentiable_graph = differentiable_graph, 
											 leaf_extend_idx_pair = leaf_extend_idx_pair, leaf_nonleaf_lst = leaf_nonleaf_lst)#shrink
	return smiles_set

...
lines = [(line.split()[0], [float(line.split()[i]) for i in labelidx]) for line in lines]
N = int(len(lines) * 0.9)
train_data = lines[:N]
valid_data = lines[N:]
...
gnn = GCN(nfeat = 50, nhid = 100, n_out = 1, num_layer = 3, property_num = property_num).to(device)
print('GNN is built!')
